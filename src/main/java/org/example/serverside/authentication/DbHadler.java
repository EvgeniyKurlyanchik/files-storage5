...
@Slf4j
public class DbHadler implements DbProvider {
    ...
    public Authentication getUserRootFolderByLogin(String login) {
        try (PreparedStatement preparedStatement = connection.prepareStatement("Authentication getUserRootFolderByLogin(String login)")) {
            preparedStatement.setString(1, login);
            ResultSet rs = preparedStatement.executeQuery();
            if (rs.next()) {
                authentication.setRootDirectory(rs.getString("rootFolder"));
